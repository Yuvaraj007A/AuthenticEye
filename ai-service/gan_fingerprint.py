...
class GANFingerprintDetector:
    ...
    def predict(self, pil_img: Image.Image) -> dict:
        """
        Returns GAN fingerprint probability using ONLY statistical signals.
        The CNN is not fine-tuned on deepfake data, so we rely entirely on
        the forensic noise statistics which work without training.
        """
        noise_img = self._extract_residual_noise(pil_img)
        noise_arr = np.array(noise_img)
        stats = self._compute_noise_statistics(noise_arr)

        # Cross-channel correlation is NOT reliable: smooth images (real photos,
        # gradients) also score 0.9+ because all channels change together.
        # Skip it as the primary signal.

        # Kurtosis of residual noise is more diagnostic:
        #   Real camera noise: kurtosis ~ 2.5-5.5  (near-Gaussian to mildly peaky)
        #   GAN/AI residual:   kurtosis > 7-10     (super-Gaussian, very peaky)
        # Floor at 5.5 to avoid flagging real smooth-image kurtosis (measured ~5.6)
        kurtosis = stats["kurtosis"]
        kurtosis_score = min(1.0, max(0.0, (kurtosis - 5.5) / 6.0))

        # Noise energy: GAN images tend to have higher structured noise energy
        noise_e = min(1.0, stats["noise_energy"] / 3000.0)

        final_score = (0.75 * kurtosis_score) + (0.25 * noise_e)
